...
const LiveExamRequestedStatus = () => {
  ...
  const { t, ready } = useTranslation(['quiz', 'course', 'common']);
  ...
  const getUserDetailsList = useCallback(async () => {
    try {
  ...
  const onSubmit = async (data: LiveExamForm) => {
    try {
      ...
      if (response.status === 200) {
        ...
        router.replace(
          `/requested-date-details/?courseId=${router.query.courseId}&examType=${router.query.examType}&userId=${router.query.userId}`
        );
      }
    } catch (error) {
      // Handle errors
  ...
  useEffect(() => {
    ...
    getUserCourses();
  }, [getUserDetailsList, getLiveExamScheduleList, router.isReady]);
      ...
      <Container maxWidth="xl">
        ...
        >
          ...
          {userEnrollCourses?.isLiveExamDateApproved !== null ? (
            // Check if the exam date is approved
            userEnrollCourses?.isLiveExamDateApproved ? (
              // If the exam date is approved
              userEnrollCourses?.liveExamMarkes !== null ? (
                ...
                <>
                  ...
                  >
                    {t('youHaveCompleted')} {userEnrollCourses?.liveExamMarkes}
                  </Typography>
                ...
                // If the exam is pending
                <Typography>{t('waitForMarks')}</Typography>
              )
            ) : (
              // If the exam date is rejected
              <Typography>{t('rejectedRequest')}</Typography>
            )
          ) : (
            // If the exam date approval status is pending
            <Typography>{t('course:pendingRequest')}</Typography>
          )}
          ...
          >
            {userEnrollCourses && userEnrollCourses.liveExamDate
              ? getDisplayDate(userEnrollCourses.liveExamDate)
              : t('pleaseSelectAnother')}
          </Typography>
          {userEnrollCourses?.isLiveExamDateApproved === false && (
            <>
              {' '}
            ...
            <form onSubmit={handleSubmit(onSubmit)}>
              ...
              <DialogActions>
                <Button
                  type="submit"
                  variant="contained"
                  disabled={liveExamRequestLoading}
                >
                  {liveExamRequestLoading ? (
                    <>
                      {' '}
...
LiveExamRequestedStatus.individualGuard = true;

LiveExamRequestedStatus.getLayout = (page: ReactNode) => (
  <EmptyQuizLayout>{page}</EmptyQuizLayout>
);
export default LiveExamRequestedStatus;
