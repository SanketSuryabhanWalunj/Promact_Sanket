...
const CompanyPageGuard = ({ children }: { children: ReactNode }) => {
      ...
      if (userData) {
        ...
        if (parsedUserData.token) {
          ...
          if (decodedToken?.exp && decodedToken?.exp > currentUTCSeconds) {
            if (parsedUserData.roles[0].name === 'Company') {
              setLoading(false);
            } else {
              if (parsedUserData.roles[0].name === 'Individual') {
...
export default CompanyPageGuard;
