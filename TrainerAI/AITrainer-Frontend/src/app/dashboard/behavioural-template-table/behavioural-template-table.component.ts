...
@Component({
  selector: 'app-behavioural-template-table',
  templateUrl: './behavioural-template-table.component.html',
  styleUrls: ['./behavioural-template-table.component.css']
})
export class BehaviouralTemplateTableComponent {
  lists!: any[];
  ...
  readonly DeleteTemplate = StringConstant.deleteTemplate;
  ...
  getTemplateList() {
    ...
    this.userService
      .getData(
        `BehaviouralTemplate/get-Templates?currentPage=${this.currentPage}&defualtList=${this.defualtList}`
      )
      .subscribe(
        (res) => {
  ...
  deleteTemplate(id: string) {
    this.userService
      .deleteData(`BehaviouralTemplate/delete-template?Id=${id}`)
      .subscribe((res) => {
        if (res) {
          this.alert.success(this.TemplateDeleteSuccess);
  ...
  goToPage(pageNumber: any) {
    ...
    this.userService
      .getData(
        `BehaviouralTemplate/get-Templates?currentPage=${pageNumber}&defualtList=${this.defualtList}`
      )
      .subscribe((res) => {
        this.lists = [];
        this.lists = res;
  ...
  editDetails(id: any) {
    this.router.navigateByUrl(`dashboard/behaviour-template/edit/${id}`);
  }

  addtemplate() {
    this.router.navigateByUrl('dashboard/behaviour-template/create');
  }
